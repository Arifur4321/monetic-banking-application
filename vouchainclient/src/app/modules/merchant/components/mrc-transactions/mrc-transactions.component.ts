...
@Component({
  selector: 'app-mrc-transactions',
  templateUrl: './mrc-transactions.component.html',
  styleUrls: ['./mrc-transactions.component.css'],
})
export class MrcTransactionsComponent implements OnInit, OnDestroy {
  /* vvv ATTRIBUTES vvv */
  ...
  /* Models */
  transactionSender: EmployeeDTO = new EmployeeDTO();
  ...
  /* Get info about the transaction's sender */
  getTransactionSender(usrId: string) {
    this.empProfileService.getProfileDetails(usrId).subscribe((response) => {
      this.transactionSender = response;
    });
  }
  ...
  showTransactionDetails(trc: TransactionDTO) {
    ...
    if (trc.trcType === 'SPS') {
      this.getTransactionSender(trc.usrIdDa);
    }
    ...
    $('#mrc-transactions-modal-transaction-detail').modal('show');
  }
